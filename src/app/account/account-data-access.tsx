...
export function useGetWalletNfts({ address }: { address: PublicKey }) {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      return await queryOwnerNft(address.toString());
      // if (heliusApiEndpointUrl) {
      //   return await queryOwnerNft(heliusApiEndpointUrl, address.toString());
      // } else {
      //   return [];
      // }
      // return (await queryOwnerNft(connection, address.toString())) ?? [];
    },
  });
}
